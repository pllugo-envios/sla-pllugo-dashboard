...
window.GAEL_CONFIG = {
  ...
  toggle: { container: null, className: 'btn-clear', label: 'Gael', remember: true },
...
function gaelFindDriver(question) {
  ...
  for (const d of all) {
    const dn = gaelNorm(d.name);
    const s = tokens.filter(t => dn.includes(t)).length;
...
function gaelGapText() {
  ...
  return `<span class="agent-title">Gap para 95%</span>Com ${mil(a.total)} pedidos elegíveis, o SLA atual é <b>${gaelPct(a.sla)}</b>. Faltam aproximadamente <b>${mil(need)}</b> entrega(s) dentro da regra para chegar a 95%.`;
}
...
function gaelAnswerOverview() {
  ...
  return `<span class="agent-title">Resumo do Gael</span>${window.GAEL_CONFIG.summary({ ...a, transportadora:(document.querySelector('.carrier-tabs .tab.active')?.textContent || 'Pllugo').trim(), meta:95 })}`
...
function gaelAnswerWhere() {
  ...
  if (!acts.length) return '<span class="agent-title">Onde atuar agora</span>Nenhuma exceção relevante apareceu nos filtros atuais.';
  return `<span class="agent-title">Onde atuar agora</span>${acts.map((x, i) => `<div class="agent-risk-line"><b>${i + 1}. ${gaelEsc(x.title)}</b><span>${gaelEsc(x.text)}</span></div>`).join('')}<span class="agent-note">A ordem considera Torre, status, pendências, score e SLA.</span>`;
}

function gaelAnswerRecurrent() {
  const r = gaelRecurrentDrivers(7);
  return `<span class="agent-title">Reincidência · últimos 7 dias</span>`
       + (r.length ? r.slice(0, 8).map(d => `<div class="agent-risk-line"><b>${gaelEsc(d.name)}</b><span>${d.badDays} dia(s) abaixo de 95% · ${mil(gaelN(d.pendentes))} perdas</span></div>`).join('') : 'Não encontrei motoristas com 2 ou mais dias abaixo da meta no período.');
}
...
function gaelAnswerQuestion(pergunta, dados, data) {
  ...
  const driverTokenHit = driver && gaelNorm(driver.name).split(/\s+/).some(t => t.length >= 3 && q.includes(t));
  ...
  if (has('motoristas ofensores','motorista ofensor','ofensores','fora da meta','abaixo da meta')) {
    ...
    return { text:`<span class="agent-title">Motoristas ofensores</span>${mil(gaelTopOffenders(null, 999).length)} motorista(s) ofensores. Top 3 por impacto: ${top.map(d => `<b>${gaelEsc(d.name)}</b> (${gaelPct(d.sla)}, ${mil(gaelN(d.pendentes))} pend.)`).join('; ')}.`, type:'warning', point:'#ofensores-section' };
  }
  if (has('maua','mauá','ribeirao','ribeirão','unidade')) {
    ...
    return { text:`<span class="agent-title">Comparativo de unidades</span>${rows.map(x => `<div class="agent-risk-line"><b>${gaelEsc(x.name)}</b><span>${gaelPct(x.a.sla)} · ${mil(x.a.pendentes)} perdas · ${mil(x.a.total)} elegíveis</span></div>`).join('') || 'Sem base por unidade no filtro atual.'}`, type:'info', point:'#overview-grid' };
  }
  if (has('cliente','jt','j&t','imile')) {
    ...
    return { text:`<span class="agent-title">Comparativo de clientes</span>${rows.map(x => `<div class="agent-risk-line"><b>${x.name}</b><span>${gaelPct(x.a.sla)} · ${mil(x.a.pendentes)} perdas · ${mil(x.a.total)} elegíveis</span></div>`).join('') || 'Sem base por cliente no filtro atual.'}`, type:'info', point:'#overview-grid' };
  }
  if (has('causa','motivo','porque','por que','razao','razão','ocorrencia','ocorrência')) {
    ...
    return { text:`<span class="agent-title">Causas de perda</span>${gaelEsc(text || 'A quebra por causa aparece em Visão Operação > Ofensores quando houver ocorrências categorizadas.')}`, type:'info', point:'#cause-section' };
  }
  if (has('dado','qualidade','atualiz','confiar','auditoria')) {
    return { text:`<span class="agent-title">Confiabilidade da leitura</span>${gaelEsc(dados.qualidade || 'Qualidade dos dados')} · ${gaelEsc(dados.qualidadePill || '')}. ${gaelEsc(dados.frescor || '')}`, type:'info', point:'#data-quality-banner' };
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => { gaelInstallOfficialStyle(); setInterval(gaelEnhanceDock, 300); setInterval(gaelPatchSpeechPlacement, 300); });
} else {
