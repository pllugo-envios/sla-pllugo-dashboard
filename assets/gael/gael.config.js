...
window.GAEL_CONFIG = {
  ...
  /* ================================================================
     ...
     ================================================================ */
  character: {
    heightVh: 0.20,      // altura como fração da janela
    heightMin: 112,      // nunca menor que isso (px)
    heightMax: 220,      // nunca maior que isso (px)
    entry: 72,           // entra e repousa mais próximo da borda esquerda
  ...
  */
  ...
  /* ================================================================
     ...
     Desativadas por decisão operacional: o Gael só analisa, fala e
     direciona telas quando o usuário envia um comando.
     ================================================================ */
  watchers: []
};
...
function gaelEnhanceDock() {
  ...
  if (dock.dataset.syncBound !== '1') {
    ...
    document.addEventListener('change', e => {
      if (e.target.matches('#unit-filter,#date-start,#date-end')) syncSoon();
    }, true);
...
function gaelPatchFixedLeftBehavior() {
  ...
  gael.homeX = function() {
    const b = this.bounds();
    return Math.min(b.max, Math.max(b.min, b.min + 10));
  };
...
function gaelInstallFilterSync() {
  ...
  document.addEventListener('change', e => {
    if (e.target.matches('#unit-filter,#date-start,#date-end')) hideSpeech();
  }, true);
}
